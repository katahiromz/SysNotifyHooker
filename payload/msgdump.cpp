...
std::string& md_hwnd2string(HWND hwnd)
{
    return reinterpret_cast<MSGDUMP *>(hwnd)->m_str;
}

BOOL md_hwnd2uni(HWND hwnd)
{
    return reinterpret_cast<MSGDUMP *>(hwnd)->m_bUnicode;
}
...
std::string md_dump(BOOL bUnicode, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    MSGDUMP msgdump;
    msgdump.m_bUnicode = bUnicode;
    HWND hwnd = reinterpret_cast<HWND>(&msgdump);
    md_process(hwnd, uMsg, wParam, lParam);
    return msgdump.m_str;
}
